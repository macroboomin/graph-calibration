...
G_large = nx.read_gpickle("./bio/knowledge_graph.gpickle")
...
for (head, tail, relation), score in predicted_scores.items():
    if G_large.has_edge(head, tail):
        print(f"Edge ({head}, {relation}, {tail}) is likely to be true with a score of {score}")
    else:
        print(f"Edge ({head}, {relation}, {tail}) is less likely with a score of {score}")
